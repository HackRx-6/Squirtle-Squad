        ...
        return await sentryMonitoringService.track(
            ...
            async () => {
                ...
                try {
                            ...
                            if (riskAssessment.risk === "critical") {
                                console.warn(
                                    `🚨 Critical risk question detected and blocked:`,
                                    {
                                        ...
                                        detectedPatterns:
                                            riskAssessment.detectedPatterns,
                                        questionPreview:
                                ...
                                console.warn(
                                    `⚠️ High risk question sanitized:`,
                                    {
                                        questionIndex: i + 1,
                                        riskScore: riskAssessment.score,
                                        detectedPatterns:
                    ...
                    console.log(
                        `📄 Processing document from URL: ${documents}`
                    );
                    console.log(
                    ...
                    if (nonDocUrl) {
                        ...
                        const webPrep =
                            await webContextService.enrichContextWithWebContent(
                                {
                                    question:
                                        augmentedQuestions[0] || documents,
                                    retrievedChunks: [],
                                    timerAbort:
                    ...
                    const questionEmbeddingsPromise =
                        sentryMonitoringService.track(
                            ...
                            async () => {
                                ...
                                return qaService.preGenerateQuestionEmbeddings(
                                    questions
                                );
                            },
                            {
                    ...
                    try {
                        const [downloadResult, embeddingResult] =
                            await Promise.all([
                    ...
                    try {
                        ...
                        if (processedDocument.documentType === "image") {
                            ...
                            try {
                                answers =
                                    await imageQAService.answerQuestionsWithOCRText(
                                        questions,
                                ...
                                if (isTimeout) {
                                    console.warn(
                                        "⏰ Image Q&A processing timed out"
                                    );
                                    answers = questions.map(
                                        () =>
                            ...
                            */
                            ...
                            console.log(
                                `⏰ Timer status: ${elapsedSeconds.toFixed(
                                    1
                                )}s elapsed, ${remainingSeconds.toFixed(
                                    1
                                )}s remaining`
                            );
                    ...
                    console.error(
                        "Error in HackRX processing endpoint:",
                        error
                    );
                ...
                try {
                    ...
                    requestBodyForLogging = {
                        pdf: fileEntry
                            ? {
                                  name:
                                      (fileEntry as BunFile).name ||
                                      "uploaded-file.pdf",
                                  size: (fileEntry as BunFile).size || 0,
                                  type:
                                      (fileEntry as BunFile).type || "unknown",
                              }
                            : null,
                    ...
                    if (timerContext.isExpired) {
                        ...
                        return new Response(
                            JSON.stringify({
                                message:
                                    "Processing timed out during file validation",
                                timeout: true,
                                elapsed_seconds:
                                    (Date.now() - timerContext.startTime) /
                                    1000,
                    ...
                    const documentService = new DocumentProcessingService();
                    const processedDocument =
                        await documentService.processDocument(buffer, fileName);
                    ...
                    if (questions.length > 0) {
                        ...
                        if (processedDocument.documentType === "image") {
                            ...
                            try {
                                ...
                                if (isTimeout) {
                                    console.warn(
                                        "⏰ Full-content Q&A processing timed out"
                                    );
                                    answersArray = questions.map(
                                        () =>
                                            "I apologize, but I wasn't able to complete the response within the time limit. Please try again with a more specific question."
                            ...
                            try {
                                ...
                                // Use the streaming results
                                answersArray = streamingAnswers.map(
                                    (answer) => {
                                        // Clean up the text response by normalizing whitespace and line breaks
                                        return answer
                                            .replace(/\n+/g, " ") // Replace multiple newlines with single space
                    ...
                    */
                    ...
                    const response = {
                        message:
                            "PDF processed successfully (no questions provided)",
                    };
