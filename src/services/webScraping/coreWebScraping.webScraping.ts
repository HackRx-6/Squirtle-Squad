...
export class CoreWebScrapingService {
            ...
            if (reader) {
                ...
                while (true) {
                    ...
                    if (value) {
                        ...
                        if (received > (tc.advanced?.maxBytes || 2000000))
                            break;
                        chunks.push(value);
                    }
                }
    ...
    private htmlToText(html: string): string {
        // Simple sanitizer and tag stripper; replace with cheerio/readability if needed later
        return html
            .replace(/<script[\s\S]*?<\/script>/gi, " ")
            .replace(/<style[\s\S]*?<\/style>/gi, " ")
            .replace(/<\/?(?!p|br|li|ul|ol|h\d)\w+[^>]*>/gi, " ")
            .replace(/<br\s*\/?>/gi, "\n")
            .replace(/<p[^>]*>/gi, "\n")
            .replace(/<li[^>]*>/gi, "\n- ")
            .replace(/<\/\w+>/g, " ")
