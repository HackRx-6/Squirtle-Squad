...
export class WebContextService {
    ...
    private rankAndLimitUrls(
        ...
        console.log(`📏 maxUrlsPerQuery: ${toolCfg?.advanced?.maxUrlsPerQuery}, maxUrlsFromDocs: ${toolCfg?.advanced?.maxUrlsFromDocs}`);

        // Rank by relevance and limit count
        const ranked = urlDetectionService
            .rankUrlRelevance(
                deduped.map((c) => c.url),
                question
            );
        ...
        const maxUrls = (toolCfg?.advanced?.maxUrlsPerQuery || 2) + (toolCfg?.advanced?.maxUrlsFromDocs || 3);
    ...
    private async fetchWebPages(
        ...
        for (const c of rankedUrls) {
            ...
            try {
                ...
                const page = await coreWebScrapingService.fetchText(
                    c.url,
                    timerAbort
                );
                console.log(`✅ Fetched URL: ${c.url}, status: ${page.status}, text length: ${page.text.length}`);
                if (page.text && page.text.length > 0) {
                    ...
                    console.warn(`⚠️ No text content from URL: ${c.url} (status: ${page.status})`);
                }
            } catch (error) {
