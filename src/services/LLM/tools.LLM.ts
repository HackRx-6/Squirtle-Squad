                ...
                parameters: {
                    ...
                    properties: {
                        urls: {
                            ...
                            items: { type: "string" },
                            description:
                                "List of absolute URLs to fetch (http/https).",
                        },
                        headers: {
                            type: "object",
                ...
                parameters: {
                    ...
                    properties: {
                        ...
                        headers: {
                            type: "object",
                            description:
                                "Optional HTTP headers to include for all requests",
                            additionalProperties: { type: "string" },
                        },
                        matchKeys: {
                            type: "array",
...
export const executeToolCall = async (
    ...
    try {
        ...
        console.log(
            `🛠️ [ToolCall:start] name=${name} args=${previewString(args)}`
        );
        switch (name) {
                    ...
                    for (const url of endpoints) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                body = {
                                    _nonJsonTextPreview: text.substring(
                                        0,
                                        8000
                                    ),
                                };
                            ...
                            const matched = doesBodyMatchCity(
                                body,
                                cityLower,
                                matchKeys
                            );
            ...
            default:
                console.warn(
                    `⚠️ [ToolCall:unknown] name=${name} args=${previewString(
                        rawArgs
                    )}`
                );
                return JSON.stringify({
...
export const runWithToolsIfRequested = async (
    ...
    if (!toolEnabled) {
        console.log(
            `🧰 Tools disabled or unavailable. Making direct LLM call.`
        );
    ...
    for (let iteration = 0; iteration < maxLoops; iteration++) {
        ...
        if (!toolCalls.length) {
            console.log(
                `🧠 Assistant provided final response without tool calls.`
            );
            ...
            const fallbackContent =
                fallbackResp.choices[0]?.message?.content?.trim();
        ...
        const toolResults = await Promise.allSettled(
            toolCalls.map(async (toolCall) => {
                ...
                try {
                    ...
                    console.log(
                        `📥 Tool '${toolName}' completed: ${previewString(
                            result
                        )}`
                    );
                    return {
