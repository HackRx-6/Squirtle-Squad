...
export class StreamingService {
        ...
        return new ReadableStream({
          start: (controller) => {
            ...
            if (timerContext.isExpired) {
              this.completeStreamWithTimeout(streamId, controller);
              return;
            }

            // Listen for timer expiration
            const originalAbort = timerContext.abortController.abort.bind(
      ...
      async () => {
        ...
        sentryMonitoringService.addBreadcrumb({
          ...
          data: {
            ...
            secondaryModel,
            questionIndex,
            globalTimeoutSeconds: qaConfig.globalTimer.timeoutSeconds,
        ...
        return new ReadableStream({
          start: (controller) => {
            ...
            timerContext.abortController.abort = () => {
              this.logger.warn("Timer expired during parallel streaming", {
                streamId,
              });
              this.completeStreamWithTimeout(streamId, controller);
              originalAbort();
      ...
      async () => {
        ...
        const createStreamProcessor = async (
          ...
          try {
            ...
            const userRisk =
              PromptInjectionProtectionService.calculateRiskScore(userMessage);
            const systemRisk =
              PromptInjectionProtectionService.calculateRiskScore(systemPrompt);
            ...
            if (
              ...
              finalUserMessage = PromptInjectionProtectionService.sanitizeText(
                userMessage,
                {
                  strictMode: true,
            ...
            if (systemRisk.score > 40) {
              ...
              finalSystemPrompt = PromptInjectionProtectionService.sanitizeText(
                systemPrompt,
                {
                  strictMode: true,
                  preserveFormatting: false,
            ...
            for await (const chunk of stream) {
              ...
              if (
                ...
                this.logger.info(`${clientType} stream won the race`, {
                  streamId,
                  model,
              ...
              if (
                ...
                const qaConfig = Config.app.getQAConfig();
                if (
                  streamContext.buffer.length >= qaConfig.streaming.bufferSize
                ) {
              ...
              if (
                ...
                this.logger.info(`${clientType} stream completed normally`, {
                  streamId,
            ...
            if (
              ...
              (clientType === "primary" && !this.activeStreams.get(streamId))
            ) {
              throw error;
            }
          }
        ...
        try {
          ...
          const promises = [
            createStreamProcessor(primaryClient, primaryModel, "primary"),
            createStreamProcessor(secondaryClient, secondaryModel, "secondary"),
          ];
            ...
            if (streamContext.timerContext.isExpired) {
              ...
              if (primaryBuffer.length > 0) {
                ...
                sentryMonitoringService.addBreadcrumb({
                  ...
                  category: "parallel_streaming_fallback_success",
                  data: {
                    streamId,
                    primaryModel,
                    fallbackContentLength: primaryBuffer.length,
                    fallbackStrategy: "primary_buffer",
                  },
                });
              } else {
                ...
                sentryMonitoringService.addBreadcrumb({
                  message: "🔄 Attempting non-streaming primary fallback",
                  category: "parallel_streaming_nonstreaming_fallback",
                  level: "warning",
                  data: {
                    streamId,
                    primaryModel,
                    reason: "no_primary_buffer_content",
                    fallbackStrategy: "non_streaming_primary",
                  },
                });
                ...
                try {
                  ...
                  sentryMonitoringService.addBreadcrumb({
                    message: "✅ Non-streaming primary fallback successful",
                    category: "parallel_streaming_fallback_success",
                    data: {
                      streamId,
                      primaryModel,
                  ...
                  sentryMonitoringService.addBreadcrumb({
                    message: "❌ Non-streaming primary fallback failed",
                    category: "parallel_streaming_fallback_failure",
                    level: "error",
                    data: {
                      streamId,
                      primaryModel,
              ...
              this.logger.warn("No winner selected in parallel streaming", {
                streamId,
                primaryFinished,
                secondaryFinished,
              });
              ...
              // Track race failure
              sentryMonitoringService.addBreadcrumb({
                message: "❌ No winner in parallel streaming race",
              ...
              this.completeStreamWithError(
                streamId,
                controller,
                new Error("Both parallel streams failed to produce content")
              );
            }
          ...
          this.logger.error("Error in parallel streaming coordination", {
            streamId,
            error: error instanceof Error ? error.message : String(error),
          });
    ...
    await sentryMonitoringService.track(
      ...
      async () => {
        try {
          ...
          for await (const chunk of stream) {
            const streamContext = this.activeStreams.get(streamId);
            if (!streamContext || streamContext.timerContext.isExpired) {
              this.logger.warn("Stream stopped due to timer expiration", {
                streamId,
              });
              break;
            }
            ...
            if (chunk.choices[0]?.finish_reason) {
              this.logger.info("LLM streaming completed normally", {
                streamId,
                finishReason: chunk.choices[0].finish_reason,
                totalLength: streamContext.buffer.length,
              });
  ...
  public getStats(): {
    ...
    const now = Date.now();
    const streams = Array.from(this.activeStreams.values()).map((context) => ({
      id: context.id,
