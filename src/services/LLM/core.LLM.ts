...
export class LLMService implements LLMProvider {
    ...
    constructor() {
        ...
        if (
            aiConfig.hasClaudeConfigured() &&
            this.claudeConfig.primary.apiKey
        ) {
    ...
    public async generateClaudeResponse(
        ...
        const result = await sentryMonitoringService.track(
            ...
            async () => {
                try {
                    this.logger.debug(
                        "Starting Claude response generation for Excel",
                        {
                            model: this.claudeConfig.primary.model,
                            systemPromptLength: systemPrompt.length,
                            userMessageLength: userMessage.length,
                    ...
                    if (!claudeClient) {
                        throw new Error(
                            "No Claude client available for Excel processing"
                        );
                    }
                    ...
                    if (!response.choices[0]?.message?.content) {
                        const errorMsg =
                            "No response content from Claude service";
                    ...
                    this.logger.info(
                        "Claude response generated successfully for Excel",
                        {
                            model: this.claudeConfig.primary.model,
                    ...
                    this.logger.error(
                        "Failed to generate Claude response for Excel",
                        {
                            error:
                                error instanceof Error
                                    ? error.message
                    ...
                    if (
                        this.claudeSecondaryClient &&
                        this.claudePrimaryClient &&
                        error
                    ) {
                        console.log(
                            "🔄 Retrying with secondary Claude client..."
                        );
                        try {
                            ...
                            if (
                                secondaryResponse.choices[0]?.message?.content
                            ) {
                                const content =
                                    secondaryResponse.choices[0].message
                                        .content;
                                console.log(
    ...
    async generateStreamingResponseForExcel(
        ...
        try {
            ...
            return new ReadableStream({
                async start(controller) {
                    try {
                        ...
                        console.error(
                            "❌ Claude streaming error for Excel:",
                            error
                        );
            ...
            console.error(
                "❌ Failed to start Claude streaming for Excel:",
                error
            );
            throw error;
        }
    ...
    private async generateStreamingResponseWithDocumentType(
        ...
        console.log(
            `🌊 Generating streaming response with load-balanced LLM...`
        );
    ...
    async generateParallelStreamingResponse(
        ...
        if (isRacingEnabled) {
            ...
            return await sentryMonitoringService.track(
                ...
                async () => {
                    this.logger.info(
                        "Starting LLM racing with both LLMs (EXPENSIVE)",
                        {
                            primaryModel: this.primaryConfig.model,
                            secondaryModel: this.secondaryConfig.model,
                            systemPromptLength: systemPrompt.length,
                ...
                if (isRacingEnabled) {
                    ...
                    for (let i = 0; i < systemPrompts.length; i++) {
                        ...
                        if (systemPrompt && userMessage) {
                            ...
                            const raceStream =
                                streamingService.streamParallelLLMResponse(
                                    this.primaryClient,
                                    this.primaryConfig.model,
                                    this.secondaryClient!,
                                    this.secondaryConfig.model,
                    ...
                    const firstHalfSystemPrompts = systemPrompts.slice(
                        0,
                        midpoint
                    );
                    const firstHalfUserMessages = userMessages.slice(
                        0,
                        midpoint
                    );
                    const secondHalfSystemPrompts =
                        systemPrompts.slice(midpoint);
                    ...
                    const firstHalfPromises = firstHalfSystemPrompts.map(
                        async (systemPrompt, index) => {
                            ...
                            if (!systemPrompt || !userMessage) {
                                throw new Error(
                                    `Missing prompt or message at index ${index}`
                                );
                            }
                    ...
                    const secondHalfPromises = secondHalfSystemPrompts.map(
                        async (systemPrompt, index) => {
                            ...
                            if (!systemPrompt || !userMessage) {
                                throw new Error(
                                    `Missing prompt or message at index ${
                                        midpoint + index
                                    }`
                                );
                            }
                    ...
                    const [firstHalfStreams, secondHalfStreams] =
                        await Promise.all([
    ...
    private async processBatchWithSingleClient(
        ...
        return await sentryMonitoringService.track(
            ...
            async () => {
                ...
                const llmPromises = systemPrompts.map(
                    async (systemPrompt, index) => {
                        ...
                        return await sentryMonitoringService.track(
                            ...
                            "llm",
                            {
                                system_prompt_length: systemPrompt.length,
                                user_message_length: userMessage.length,
                                system_prompt_preview: systemPrompt.substring(
                                    0,
                                    200
                                ),
                                user_message_preview: userMessage.substring(
                                    0,
                                    200
                                ),
                                index,
                            ...
                            async () => {
                                console.log(
                                    `🤖 [${clientType}] Processing LLM request ${
                                        index + 1
                                    }/${systemPrompts.length}`
                                );

                                const textOutput =
                                    await runWithToolsIfRequested(
                                        client,
                                        config.model,
                                        systemPrompt,
                                        userMessage,
