...
export class TextCleaningService {
  ...
  public static cleanText(
    ...
    if (enablePromptInjectionProtection) {
      ...
      cleanedText = PromptInjectionProtectionService.sanitizeText(text, {
        strictMode: strictSanitization || riskAssessment.risk === "critical",
  ...
  public static cleanFullText(
    ...
    console.log(`✅ Full text cleaning completed in ${endTime - startTime}ms`);
  ...
  public static sanitizeForAI(
    ...
    console.log(`🛡️ Sanitizing ${documentType} document for AI processing...`);
    const startTime = Date.now();

    // Step 1: Initial risk assessment
    const initialRisk =
      PromptInjectionProtectionService.calculateRiskScore(documentContent);

    // Step 2: Apply document-specific sanitization
    // If prompt-injection protection is disabled in config, skip calling
    // into the protection service entirely to avoid misleading logs and
    // repeated no-op processing. We still keep the initial risk
    // assessment so callers receive consistent information.
    ...
    if (PromptInjectionProtectionService.isEnabled()) {
      ...
      console.log(
        "🔓 Prompt injection protection is disabled — skipping document-level sanitization"
      );
      sanitizationResult = {
        sanitizedContent: documentContent,
