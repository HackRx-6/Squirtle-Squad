...
import { urlDetectionService, webContextService } from "../webScraping";
...
export class InMemoryQAService {
    ...
    private async storeChunksWithPrecomputedEmbeddings(
        ...
        return await sentryMonitoringService.track(
            ...
            "vector_search",
            {
                ...
                successful_embeddings: embeddings.filter((e) => e !== null)
                    .length,
            ...
            async () => {
                ...
                const searchConfig =
                    this.configService.getQAConfig().vectorSearch;
    ...
    async answerMultipleQuestionsWithPrecomputedEmbeddings(
        ...
        return await sentryMonitoringService.track(
            ...
            async () => {
                ...
                const qaConfig = this.configService.getQAConfig();
                const dynamicChunksToLLM =
                    this.configService.getDynamicChunksToLLM(this.totalPages);

                const contextPromises = questions.map(
                            ...
                            await sentryMonitoringService.track(
                                ...
                                "vector_search",
                                {
                                    ...
                                    dynamic_chunks_enabled:
                                        qaConfig.dynamicChunking.enabled,
                                },
                                async () => {
                ...
                if (
                    ...
                    const systemPrompts = contextResults.map(
                        (ctx) => ctx.systemPrompt
                    );
                    const userMessages = contextResults.map(
                        (ctx) => ctx.userMessage
                    );
                    ...
                    try {
                        ...
                        answers = await this.generateAnswersIndividually(
                            contextResults
                        );
                    }
                } else {
                ...
                const results: QuestionAnswer[] = questions.map(
                    (question, index) => ({
                        question,
                        answer:
                            answers[index] ||
    ...
    async answerMultipleQuestions(
        ...
        return await sentryMonitoringService.track(
            ...
            "qa_pipeline",
            {
                question_count: questions.length,
                file_name: fileName,
                embedding_load_balancing:
                    this.embeddingService.isLoadBalancingActive(),
                llm_load_balancing: this.llmService.isLoadBalancingActive(),
                total_question_length: questions.reduce(
                    (sum, q) => sum + q.length,
                    0
                ),
                avg_question_length: Math.round(
                    questions.reduce((sum, q) => sum + q.length, 0) /
                        questions.length
                ),
            },
            async () => {
                ...
                if (
                    ...
                    try {
                        ...
                        // Fallback to individual processing
                        queryEmbeddings =
                            await this.generateEmbeddingsIndividually(
                                questions
                            );
                    }
                } else {
                ...
                const contextPromises = successfulQuestions.map(
                    async (question, successIndex) => {
                        const queryEmbedding =
                            validQueryEmbeddings[successIndex]!;
                ...
                questions.forEach((originalQuestion, originalIndex) => {
                    const successIndex =
                        questionIndexMap.indexOf(originalIndex);
                    ...
                    if (successIndex !== -1 && answers[successIndex]) {
                        ...
                        completeResults.push({
                            question: originalQuestion,
                            answer: "I couldn't process this question due to a timeout during embedding generation. Please try rephrasing your question or contact support if this persists.",
                        });
                    }
                });
                        ...
                        return await sentryMonitoringService
                            .track(
                                ...
                                "embedding",
                                {
                                    ...
                                    question_preview: question.substring(
                                        0,
                                        100
                                    ),
                                    question_index: index,
                        ...
                        try {
                            return await sentryMonitoringService.track(
                                ...
                                },
                                {
                                    component: "load_balanced_qa_service",
                                    operation:
                                        "individual_answer_generation_fallback",
                                }
                            );
                        } catch (error) {
                            console.error(
                                `❌ Failed to generate answer for question ${
                                    index + 1
                                }:`,
                                error
                            );
    ...
    async answerQuestion(
        ...
        const results = await this.answerMultipleQuestions(
            [question],
            fileName
        );
        return results[0]!;
        ...
        return await sentryMonitoringService.track(
            ...
            async () => {
                ...
                if (timerContext.isExpired) {
                    console.warn(
                        "⚠️ Timer already expired before processing started"
                    );
                ...
                try {
                    if (timerContext.isExpired) {
                        throw new Error(
                            "Timer expired during embedding generation"
                        );
                    }
                    ...
                    if (
                        ...
                        const embeddingPromises = questions.map(
                            async (question, index) => {
                                ...
                                try {
                                    ...
                                    console.warn(
                                        `⚠️ Failed to generate embedding for question ${
                                            index + 1
                                        }`
                                    );
                                    return null;
                    ...
                    console.error(
                        "❌ Failed to generate question embeddings:",
                        error
                    );
                    return questions.map(
                        () =>
                ...
                const contextPromises = questions.map(
                    async (question, index) => {
                        ...
                        try {
                            // Find similar chunks
                            const similarChunks =
                                await this.vectorService.findSimilar(
                                    embedding,
                                    dynamicChunksToLLM
                                );
                            ...
                            try {
                                const qaConfig2 =
                                    this.configService.getQAConfig();
                                if (qaConfig2.toolCalls?.enabled) {
                                    const intent =
                                        urlDetectionService.classifyUrlIntent(
                                            question
                                        );
                            ...
                            const systemPrompt =
                                this.configService.getQAConfig().toolCalls
                                    ?.enabled
                            ...
                            console.error(
                                `❌ Error preparing context for question ${
                                    index + 1
                                }:`,
                                error
                            );
                ...
                if (systemPrompts.length > 0) {
                    try {
                        if (this.llmService.isLoadBalancingActive()) {
                            ...
                            const streamTextPromises = streams.map(
                                async (stream, index) => {
                                    try {
                                        return await streamingService.streamToText(
                                            stream
                                        );
                                    } catch (error) {
                                        console.error(
                            ...
                            streamResults = await Promise.all(
                                streamTextPromises
                            );
                            ...
                            const streamPromises = systemPrompts.map(
                                async (systemPrompt, index) => {
                                    ...
                                    try {
                                        ...
                                        console.error(
                                            `❌ Error processing individual stream ${
                                                index + 1
                                            }:`,
                                            error
                                        );
                        ...
                        console.error(
                            "❌ Error in batch streaming processing:",
                            error
                        );
                ...
                questions.forEach((question, originalIndex) => {
                    ...
                    if (contextResult?.error) {
                        ...
                        if (
                            contextResult.error ===
                            "Timer expired during context preparation"
                        ) {
                            finalResults.push(
                                "I apologize, but the request timed out during processing."
                            );
                        } else if (
                            contextResult.error === "No embedding available"
                        ) {
                            finalResults.push(
                                "I couldn't process this question due to an embedding generation error."
                            );
                        } else if (
                            contextResult.error === "No relevant context found"
                        ) {
                            finalResults.push(
                        ...
                        const streamIndex =
                            contextQuestionMap.indexOf(originalIndex);
