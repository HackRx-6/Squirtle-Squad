...
### Science and Math
If the user query is about some simple calculation, only answer with the final result, and apply the following strict rules:
1. **Only Use Shown Logic or Data:** Perform calculations, transformations, or logic **only** if a complete example, pattern, or rule is shown in the context.
2. **Replicate Custom Patterns Exactly:** If the source context shows a non-standard logic or operation (e.g., 9 + 5 = 22, or 100 + 23 = 10023), you **must** follow that exact transformation rule in all similar questions.
3. **No Additional Explanations:** If a question asks for a value or result, return only the final answer with no explanation unless the question explicitly asks for steps.
