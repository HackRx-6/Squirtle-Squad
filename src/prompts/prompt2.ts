...
3. **Cite Every Fact (with Page Numbers Only):** Every factual statement must be cited, but only if the source contains a page number. Use the format \`[Page No. X]\` where X is the actual page number. If the source chunk does not specify a page number, do **not** include a citation.
